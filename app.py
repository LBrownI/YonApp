...
def parse_schedule_row(row):
    ...
    if inicio.endswith('.0'): inicio = inicio[:-2]
    if fin.endswith('.0'): fin = fin[:-2]
...
def process_schedule(file_path):
    try:
        ...
        if 'materia' not in df.columns or 'ubicacion' not in df.columns:
            return None, f"Error: El Excel no tiene las columnas 'NOMBRE' o 'SALA'. Columnas detectadas: {list(df.columns)}"
